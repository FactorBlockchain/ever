...
export class FeedService extends Extender {
			...
			.pipe(
				switchMap((_feed: IFeed[]) => {
					...
					this.feed.forEach((item: IFeed) => {
						reads.push(
							this.firestoreService.doc$(`users/${item.uid}`)
						);
					});
					return combineLatest(reads);
				}),
				map((data: any) => {
					return (this.feed = this.feed.map(
						(item: IFeed, index: number) => {
							return {
								...item,
								userLiked:
									item.likes && isArray(item.likes)
	...
	public getUserFeed(uid) {
		return this.firestoreService.colWithIds$<IFeed>(`feed`, (ref: any) =>
			ref.where('uid', '==', uid)
		);
	}
		...
		return this.firestoreService.doc$(`feed/${pid}`).pipe(
			...
			map((arr: any) => {
				...
				return {
					...
					...joins,
					userLiked:
						feed.likes && isArray(feed.likes)
							? feed.likes.includes(uid)
							: false
				};
			})
		);
		...
		if (val && val.trim() !== '') {
			return feed.filter((item) => {
				return (
					...
					item.subtitle.toLowerCase().indexOf(val.toLowerCase()) >
						-1 ||
					item.content.toLowerCase().indexOf(val.toLowerCase()) >
						-1 ||
	...
	public getComments(fid: string, uid: string) {
		return this.firestoreService
			.colWithIds$<IComment>(`feed/${fid}/comments`, (ref: any) =>
				ref.limit(20)
			)
			.pipe(
				...
				map((data: any) => {
					return (this.comments = this.comments.map(
						(item: IComment, index: number) => {
							return {
								...item,
								user: data[index] || null
							};
	...
	public async upsertComment(comment: IComment) {
		...
		await this.firestoreService.set(
			`feed/${comment.fid}/comments/${comment.id}`,
			comment
		);
