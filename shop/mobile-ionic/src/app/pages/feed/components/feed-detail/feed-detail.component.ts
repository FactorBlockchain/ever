...
export class FeedDetailComponent extends Extender implements OnInit {
			...
			buttons: [
				...
				},
				{
					text: this.translate.instant(
						'feed-detail-component.open-comments'
					),
					handler: () => {
						this.comment(feed);
					}
				...
				{
					text: this.translate.instant(
						'feed-detail-component.share-button'
					),
					handler: () => {
						this.share(feed);
					}
	...
	public async edit(post: IFeed) {
		if (post.user.uid === this.user.uid) {
			...
			this.toast(
				this.translate.instant('feed-detail-component.cannot-edit')
			);
		}
	}
		...
		if (post.uid === this.user.uid) {
			const prompt = await this.alertCtrl.create({
				header: this.translate.instant(
					'feed-detail-component.delete-alert-title'
				),
				message: this.translate.instant(
					'feed-detail-component.delete-alert-message'
				),
				buttons: [
					{
						text: this.translate.instant('other.cancel'),
							...
							this.feedService.delete(post).then(() => {
								...
								this.toast(
									this.translate.instant(
										'feed-detail-component.confirm-delete'
									)
								);
							});
						}
			...
			this.toast(
				this.translate.instant('feed-detail-component.cannot-delete')
			);
		}
	}
