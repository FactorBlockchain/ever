...
import { CalendarEventListComponent } from 'shared/modals/calendar-event-list/calendar-event-list.component';
...
export class ScheduleComponent extends Extender implements OnInit {
	...
	public async view(day: IDay): Promise<any> {
		const modal = await this.openModal(CalendarEventListComponent, {
			data: day
		});
		await modal.present();
	}
