...
export class ScheduleService extends Extender {
	...
	public getUserEvents(uid: string): Observable<any> {
		return this.firestoreService.colWithIds$('events', (ref) =>
			ref.where('uid', '==', uid)
		);
	}
	...
	public updateEvent(data: ICalendarEvent) {
		data.start = !Number(data.start)
			? Date.parse(data.start as string)
			: data.start;
		data.end = !Number(data.end)
			? Date.parse(data.end as string)
			: data.end;
		return this.firestoreService.update<ICalendarEvent>(
			`events/${data.id}`,
			data
		);
	}
