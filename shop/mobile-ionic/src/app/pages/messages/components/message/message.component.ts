...
import { INoData } from 'shared/components/no-data/no-data.component';
import { Extender } from 'shared/helpers/extender';
import { GalleryPickerComponent } from 'shared/modals/gallery-picker/gallery-picker.component';
...
export class MessageComponent extends Extender
	...
	public async ngOnInit() {
		...
		this.subscriptions.push(
			this.messageService
				.getMessage(this.navParams.get('data'))
				.subscribe(
					(msg) => {
						this.message = msg;
	...
	public getSender(message: IMessage) {
		return message
			? message.participants.find(
					(user: IUser) => user.uid !== this.currentUser.uid
			  )
			: null;
	}
	...
	public call() {
		this.commonService.callUser(
			this.getSender(this.message).mobile ||
				this.getSender(this.message).phone,
			this.callNumber
		);
	}
			...
			if (data.images && data.images.length > 0) {
				...
				this.toast(
					this.translate.instant(
						'message.component.no-images-selected'
					)
				);
			}
		}
