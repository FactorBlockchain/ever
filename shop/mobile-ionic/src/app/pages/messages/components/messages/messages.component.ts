...
export class MessagesComponent extends Extender implements OnInit {
	...
	public async ngOnInit() {
		...
		const queryObservable = this.view$.pipe(
			switchMap((view) =>
				this.messageService.getMessages(this.currentUser.uid, view)
			)
		);
	...
	public getSender(message: IMessage) {
		return message.participants.find(
	...
	public async messagesOptions() {
		const asCtrl = await this.actionSheetCtrl.create({
			header: this.translate.instant(
				'messages-component.manage-messages'
			),
			buttons: [
				{
