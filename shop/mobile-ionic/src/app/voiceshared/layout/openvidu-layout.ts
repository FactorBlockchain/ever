...
export interface VoiceOpenViduLayoutOptions {
	maxRatio: number;
	minRatio: number;
	fixedRatio: boolean;
	animate: any;
...
export class VoiceOpenViduLayout {
	private layoutContainer: HTMLElement;
	private opts: VoiceOpenViduLayoutOptions;

	private fixAspectRatio(elem: HTMLVideoElement, width: number) {
		const sub: HTMLVideoElement = <HTMLVideoElement>(
			elem.querySelector('.OT_root')
		);
		if (sub) {
		...
		if (totalRowHeight < HEIGHT && remainingShortRows > 0) {
			...
			for (let i = 0; i < rows.length; i++) {
				...
				if (row.width < WIDTH) {
					...
					row.width += Math.floor(
						(extraHeight / row.height) * row.width
					);
			...
			for (let j = 0; j < row.children.length; j++) {
				...
				if (fixedRatio) {
					targetWidth = Math.floor(
						targetHeight / this.getVideoRatio(elem)
					);
				}
	...
	initLayoutContainer(container, opts) {
		this.opts = {
			...
			bigPercentage:
				opts.bigPercentage != null ? opts.bigPercentage : 0.8,
			bigFixedRatio:
				opts.bigFixedRatio != null ? opts.bigFixedRatio : false,
	...
	setLayoutOptions(options: VoiceOpenViduLayoutOptions) {
		this.opts = options;
	}
}
