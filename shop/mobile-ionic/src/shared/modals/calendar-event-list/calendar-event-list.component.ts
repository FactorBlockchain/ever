...
import { ICalendarEvent } from 'app/pages/dashboard/models/calendar-event';
import { IDay } from 'app/pages/dashboard/models/day.model';
import { ScheduleService } from 'app/pages/dashboard/services/schedule/schedule.service';
...
export class CalendarEventListComponent extends Extender implements OnInit {
	...
	public async open(day: IDay, data: ICalendarEvent = null, index: number) {
		const modal = await this.openModal(CalendarEventManageComponent, {
			day,
			data
		});
		modal.present();
