...
import { SocialShareComponent } from 'shared/modals/social-share/social-share.component';
	...
	public async share(
		...
		if ((window as any).cordova) {
			return await this.socialSharing
				...
				.then(() =>
					this.toast(
						this.translate.instant('feed-component.share-confirm')
					)
				);
		} else {
		...
		return await new Promise<string[] | any>((resolve, reject) => {
			this.imagePicker
				...
				.then((results) => {
					resolve(
						results.map(
							(item: string) => 'data:image/jpeg;base64,' + item
						)
					);
				})
				.catch((err) => {
